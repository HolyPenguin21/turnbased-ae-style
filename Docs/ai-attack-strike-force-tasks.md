...
- **Доноры из Raid/ActiveDefence.** Нужен путь освобождения чужого актёра: либо арбитраж `ActorCommitments` по `TaskScore` (сейчас удержанный актёр вообще не попадает в пул `EligibleReadyArmies`), либо приостановка операции по образцу `SuspendReason.ActiveDefencePreemption`. Плюс перевод «ценности брошенной операции» (шкала `TaskScore`) в шкалу AP, по которой ранжируются доноры. Это новый путь в жизненном цикле, его не стоит делать без игровой проверки текущих шагов.
- **Точка сбора «навстречу».** Сейчас точка = клетка одного из кандидатов в хосты (перебор всех свободных армий, минимум AP). Хост, идущий навстречу, требует новой ноги движения хоста в Gather.
- **Передача героя между армиями.** `SparableSupportBodies` не отдаёт героя донора; передача — новое правило передачи в исполнителе.
