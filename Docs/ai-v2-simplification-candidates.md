...
Проверка по логу 14:03 (15 ходов): 109 re-admission, Economy — 69 (8 с `changed=1`), пропусков
Economy по-прежнему 0. Причина уже не в шуме fingerprint: все прогоны Economy пришли с реальными
триггерами — `ResourceSite` (разведчик открыл сайт/кандидат базы, 31) или `Hand/Capability/
Resources` (сыгранная карта). В 51 из 61 прогонов с `changed=0` набор ECO-demand действительно
другой (новые сайты, сдвиг priority от ресурсов), Phase A просто не может действовать (нет карты/
строителя/AP). Гейт «тот же набор demand» поймал бы ≤10 из 61 ценой второго правила допуска —
не делаем. C4 закрыт: правка оставлена как упрощение контракта (fingerprint без глобальной версии).
