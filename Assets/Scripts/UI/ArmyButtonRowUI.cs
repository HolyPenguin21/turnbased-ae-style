...
namespace Game.UI
{
    ...
    public class ArmyButtonRowUI : MonoBehaviour
    {
        ...
        public void Show(IReadOnlyList<ArmyData> armies, Action<ArmyData> onArmyClicked, ArmyData selectedArmy = null, bool showStats = false)
        {
            ...
            // Airfield, then garrison, then everything else (per the project owner's own request) —
            // a stable sort (LINQ OrderBy, not List.Sort) so every other relative ordering the
            // caller already chose stays untouched, only airfield/garrison placement is reordered.
            _armies = _armies.OrderBy(a => a.IsAirfield ? 0 : a.IsGarrison ? 1 : 2).ToList();
