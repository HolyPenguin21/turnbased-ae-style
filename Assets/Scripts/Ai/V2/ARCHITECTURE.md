...
| `Attack` (Aggression) | `Missions/AggressionMissionPlanner` (`AppendAttack`) | One known enemy Base/Citadel per durable intent, or one known *undefended* enemy Facility (extractor/lab/factory — arrival destroys it; success = the structure is gone). `AttackMissionPhase`: Assault → Reinforcement/SupportReturn or RecoveryReturn. Attack is ground-first; successful capture retires the intent and leaves the army on the captured structure. |
