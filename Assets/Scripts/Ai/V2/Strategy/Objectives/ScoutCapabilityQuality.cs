...
namespace Game.Ai.V2
{
    ...
    //  CONTEXT the demand was raised for, it returns a bounded QUALITY MULTIPLIER applied to
    //  MaterializationCandidateBuilder.ScorePlanA's positive capability score. Every term is a
    //  MARGINAL value around 0 — "how much more useful is this than the cheap feasible alternative,
    //  HERE" — never an unconditional raw-stat bonus (spec AC2 / AC3 / AC4 / AC8).
    //
    ...
    internal static class ScoutCapabilityQuality
    {
        public struct Inputs
        {
            ...
            // The cheapest useful feasible alternative's mobility, for the MARGINAL mobility / ETA
            // compare. When this candidate is the only feasible one, pass its own MoveMax.
            public int ReferenceMoveMax;

            public ScoutCapabilityContext Context;   // may be null (no mission context propagated)

            // Hero opportunity cost exists only when this SAME Phase-A portfolio contains an active
            // Hero shortage. AvailableHeroes then tells us whether that competing use is already
            // supplied by a free deployed hero. A turn-one Hero scout is not penalised merely because
            // no hero-led field army existed before the card was played.
            public bool HasCompetingHeroDemand;
        ...
        public static float Evaluate(in Inputs x, out MaterializationQualityBreakdown bd)
        {
            ...
            // ---- ETA: turns saved reaching the focus vs the cheapest useful alternative ----
            int refEta = 1;
            if (x.HasFocus)
            {
            ...
            // ---- spot strength: only a real Scout-quality term when the objective is about
            //      re-acquiring / observing a potentially hidden target. Own detection pressure
            //      is deliberately NOT enough: spot dice detect the other side, not our scout. ----
            if (x.SpotStrength > 0)
            {
            ...
            // ---- hero opportunity cost: only a KNOWN competing Hero shortage can create it. ----
            if (x.IsHero && x.HasCompetingHeroDemand)
            {
                float scarcity;
            ...
            // ---- activation-AP drag: deploy-card AP and movement activation AP are different
            //      economic events; the latter belongs in capability quality. ----
