...
{
    // Draws one ArmyUnitEntry as a single inspector row: a card dropdown (left, built from
    // whichever NeutralArmyCatalog asset owns this property — its own `cardCatalogs` list,
    // labeled "<catalog.displayName>: <card.displayName>") plus a small copy-count field
    // (right). Same layout/idea as DeckCardEntryDrawer, just for ArmyUnitEntry instead of
    // DeckCardEntry.
    [CustomPropertyDrawer(typeof(ArmyUnitEntry))]
    public class ArmyUnitEntryDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var catalog = property.serializedObject.targetObject as NeutralArmyCatalog;
        ...
        private static List<string> BuildCardKeys(NeutralArmyCatalog catalog)
        {
