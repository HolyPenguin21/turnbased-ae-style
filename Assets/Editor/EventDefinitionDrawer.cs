...
{
    // Labels each EventDefinition row in EventCatalog's `events` list with its own `name`
    // instead of Unity's default "Element N" — same technique as CardDefinitionDrawer/
    // ArmyDefinitionDrawer (own foldout + walk the visible children, rather than a recursive
    // PropertyField which would re-enter this same drawer).
    [CustomPropertyDrawer(typeof(Game.Cards.EventDefinition))]
