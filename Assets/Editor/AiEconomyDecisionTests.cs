...
namespace Game.EditorTests
{
    public class AiEconomyDecisionTests
    {
        ...
        [Test]
        public void Allocation_SoftSurveilCommitmentRemainsProtectedFromEconomy()
        {
            var player = new Game.Players.PlayerSetupData();
            AiAllocatorStateRegistry.Clear();
            WorldSnapshot snapshot = SnapshotWithDeficits(0.8f, 0.2f, actionable: true);
            snapshot.Self.ActionPoints = 1;
            MissionProposal surveil = AllocatorMission(
                MissionKind.Scout, 20f, DesireAxis.Recon, armyId: 7);
            surveil.Target = new ScoutMissionTarget
            {
                Kind = ScoutTargetKind.Surveil,
                Contact = new EnemyContactSnapshot
                {
                    Army = new ArmySnapshot { ArmyId = 42 },
                },
            };
            MissionProposal economy = AllocatorMission(
                MissionKind.Economy, 80f, DesireAxis.Economy, armyId: 7);
            var commitment = new Commitment
            {
                IntentKey = MissionIntentKey.For(surveil), Mission = surveil,
                Tier = CommitmentTier.Soft, ContinuationValue = surveil.BaseValue,
            };

            TentativeAllocation allocation = ResourceAllocator.BeginTurn(
                snapshot, Radar.Even(), new List<MissionProposal> { surveil, economy },
                new List<Commitment> { commitment }, player).Pack();

            Assert.That(allocation.Funded.Select(x => x.Mission), Is.EqualTo(new[] { surveil }));
        }
