...
| AI-MGR-02 | End-of-turn tempo spending | DONE — rework to full TZ (not committed). `3f0c55d` (first pass) was only §7-skeleton; a stale review flagged the rest. This rework builds the **unified end-of-turn tempo arbiter**: `StrategicManager.UseSurplus` is now ONE `IEnumerator` bounded loop where PlayCard (materialization OR non-combat, scored ONLY by `StrategicCardEvaluator` NetScore — no caller-side correction, §5), DrawCard (`DrawCandidateUtility` — full peer, not a terminal fallback), ExistingStrategicSpend (`StrategicMaintenancePolicy.DescribeBest` + `StrategicPressureAdvance.BuildPlan` as candidates), HoldResources (`HoldResourcesUtility` — loose-pool value from econ fragility + resource scarcity, capped so a strong play wins), and EndTurn all compete in ONE utility space. Each iteration rebuilds snapshot/hand/resources/reservations, executes exactly one, inspects the REAL result, rebuilds; a no-op/failed candidate is parked in `rejectedForState` (cleared on any real state mutation, §2/§3); global bound `maxEndOfTurnTempoActionsPerTurn=10`; spend candidates filtered by SPENDABLE AP (§6). **Deleted**: `RunTerminalDraws` (Draw is a candidate), the HousekeepingManager post-tempo `StrategicPressureAdvance`+`StrategicMaintenancePolicy` passes (now candidates), the `CardDrawExecutor` "preserve AP for maintenance/pressure" gate, dead `surplusNonCombatReservedActions` / `StrategicMaintenancePolicy.HasPriorityAction`+`MaxActionsPerTurn` / `StrategicPressureAdvance.HasAction`. **Reaction reservation** (§7): `StrategicReactionPass.BuildReactionOpportunity` — a concrete preflight (pending + resolvable hand + real new content: discovered target OR follow-up); `Amount = min(AP, reactionReserveApCap=6)`, NOT the whole pool; only reserved when actionable, released immediately otherwise. **Ledger** (§6/§8): `Reserve`→`Upsert` idempotent by (Owner,Reason,Resource); generic `Spendable(resource)` + `Map(ResourceType)`; `AssertClearAtTurnEnd` (no reservation survives turn end) wired into `Pipeline.RunTurn`. **§13 diagnostics**: per-iteration header (resources/reservations/spendable/hand/bar) + full candidate dump + WIN line + mandatory final `strat.B/tempo — END:` line (actions, ap, spendable, H/E/M/T, reservations, stop reason). `AiStrategyV2Scope.Mode = Full` (arbiter exercised against real AGG/DEF/ECO/DEV competition). All 3 `UseSurplus` call sites → coroutine. Both Unity assemblies build clean (0/0). NOT play-tested. **Review round 2 (4×P0 + 4×P1, not committed):** (P0-1 semantic caps) `maxSurplusActionsPerTurn` un-deprecated → per-turn sub-cap on tempo *card plays* (mat + non-combat); `maxTerminalDrawsPerTurn` → per-turn sub-cap on tempo *draws*; `maxGenerationActionsPerTurn` now backed by new `StrategicGenerationBudget` (one per-(player,turn) counter) + `MaterializationReservation.GenerationAttemptsUsed` seeded from it in `FulfillDemands` AND every `UseSurplus` entry, so a fresh reservation on a reaction / Housekeeping re-run can't reset it; `maxEndOfTurnTempoActionsPerTurn` stays the wider global bound. (P0-2 second card scorer) `StrategicMaintenancePolicy` gutted to non-card actions ONLY (Base/Citadel slot-capacity upgrade); deleted `EquipmentBenefit` / `GenerationUtility` / Hero +8 / `FindPlaceableInternalFacility` / `FindEquipmentAssignment` / `FindGeneration` / `GenerationTurnState` / `MaxStandaloneGenerationAttemptsPerTurn` and config `tempoMaintenanceFacilityValue` / `tempoMaintenanceEquipmentValueScale` / `tempoMaintenanceGenerationValueScale` / `tempoMaintenanceValueCap`. Facility/Equipment/generation are ordinary PlayCard candidates via `NonCombatCardPlayer` → `StrategicCardEvaluator` (same `BuildingPlayExecutor.CanPlaceFacilityAt`). (P0-3 hidden ordering) `DescribeBest` (Facility→Capacity→Equipment→Generation fallthrough) → `EnumerateCandidates` returning independent `StrategicSpendCandidate`s, each with exact building+tier payload + `Execute()`; arbiter ranks by utility, executor runs the exact candidate (re-validates affordability only). (P0-4 stranded AP) `ExecuteRound` releases the `StrategicReactionPass` AP reservation *before* its inner `UseSurplus` so the reaction can spend the AP it held; `HousekeepingManager` runs the bounded tempo re-run whenever a reaction AP reservation existed at entry and AP remains — regardless of `reaction.Ran` (was: only when it did NOT run). (P1-5 concrete reservation) `BuildReactionOpportunity.ApRequired` = cheapest eligible field responder activation + `reactionResponderMoveApEstimate` (target-driven; no responder → `None("noEligibleResponder")`), or sampled hand-card AP / `reactionFollowupApEstimate` (follow-up only); `reactionReserveApCap` is now only the ceiling. (P1-6 generic affordability) `TempoCandidate.ResCost` full H/E/M/T vector; new `FitsSpendableResources` gates every spend candidate on `min(StrategicResourceReservationLedger.Spendable, AiResourceReservation.Available)` per resource. (P1-7 near-cap overflow) `HoldResourcesUtility` reworked to per-resource + candidate-aware (`onlyConsumed` filter — a scarce Tech no longer taxes a Materials-only play) + negative overflow term when `stock ≥ comfortable*tempoHoldOverflowStockFactor` and `PerTurnIncome > 0`; arbiter compares `eff = utility − holdOfConsumedResources`. (P1-8 Draw valuation) expected value = floor + normalised mean remaining-deck `BasePower`, thin-deck taper; hand quality graded (best current playable NetScore × weight) not boolean. Both Unity assemblies build clean (0/0) via `dotnet build`. NOT play-tested. **Review round 3 (5×P0 + 8×P1):** (P0.1 Draw starvation) the round-2 `eff = utility − hold` correction is GONE; Draw carries `ResCost = null` (AP only) so it is never penalised for holding H/E/M/T; the hand-quality penalty counts only card alternatives *selectable right now* (not ones blocked by the card-play budget / affordability / placement); `tempoDrawFillFloor`+`tempoDrawFillPerSlot` soften the fill factor and `tempoDrawFutureBlockPenalty` 0.25→0.10 so 9/10 is a legal draw. (P0.2 HoldResources semantics) restored as a REAL terminal candidate — arbiter compares `bestActionableSpend.Utility` vs `max(HoldResources, EndTurn)`, no per-action hold subtraction. (P0.3 PlayCard verbatim) `PlayCard.Utility` = `StrategicCardEvaluator` NetScore with zero caller-side correction. (P0.4 turn-scoped budget) `StrategicGenerationBudget` → `StrategicTempoBudget` — one per-(player,turn) budget for `TotalTempoActionsUsed` / `SurplusCardActionsUsed` / `DrawActionsUsed` / `GenerationAttemptsUsed`; every `UseSurplus` re-entry (main / reaction / follow-up / Housekeeping re-run) reads the same budget. (P0.5 real near-cap) `HoldResourcesUtility` per-resource `ExpectedOverflow = max(0, (stock + PerTurnIncome[r]) − softCap)` where `softCap = max(comfortable, IncomeTarget[r] * tempoHoldOverflowCapHorizon)` (no hard resource cap exists in the game — soft cap from `snap.Economy.IncomeTarget`); summed per resource so a scarce Tech can't re-inflate a Materials overflow. (P1.1/P1.2/P1.3 reaction) `StrategicReactionOpportunity` now carries `ActionKey` / `OwnerName` / `ActorId` / `ActionKind` / `ExactApRequired` / `ResourceCost` / `FailReason`; the reservation Owner = `ActionKey`, `Amount` = exact validated AP (+ exact `ResourceCost`), `reactionReserveApCap` is only a ceiling; target-driven picks a concrete cheapest eligible responder army, follow-up picks the cheapest *currently-playable* hand card (else not actionable); `ExecuteRound` revalidates the reserved owner via `ReactionOwnerStillValid` + `StrategicResourceReservationLedger.OwnerOf` and releases on mismatch. (P1.4 pre-mint cost) a generated non-combat candidate's `ResCost` = `GenerationStep.GenerationResourceCost` (the Challenge cost), not the minted stand-in's null `EffectivePlayResourceCost`. (P1.5 canonical affordability) `GenerationSource.FitsReservedAffordability` now nets out the strategic ledger too (`min(AiResourceReservation.Available, StrategicResourceReservationLedger.Spendable)`) — planning affordability == arbiter affordability. (P1.6 strategic Draw value) `GenericStrategicCardValue` = combat body power + `StrategicEffectRegistry.Roles(...)` count × `tempoDrawEffectRoleValue` + flat Equipment / Base-Facility profile (not BasePower alone). (P1.7 structured result) `TempoExecutionResult` (Succeeded / StateChanged / Progressed / Interrupt / Ap+H/E/M/T Spent / CardPlayed / Drawn / GenerationAttempted / Generated / Attached / FailReason) for EVERY tempo path; resources spent measured by the arbiter around the call. (P1.8 parking) parked keyed by `(ActionKey, StateVersion)` — a `stateVersion` bumps on any real mutation, staling every park (== full candidate rebuild). (Diagnostics) per-iteration `LogTempoIterationHeader` (per-resource total/reserved/spendable/soft-cap/income/overflow + hand/deck + budget), per-candidate `rawUtil`/`apCost`/`resCost`/block-reason, Draw breakdown (`DrawDiag`), Hold/EndTurn line, WINNER + `TempoExecutionResult` line, END line with the turn budget. Both assemblies build clean (0/0) via `dotnet build` + verified with a deliberate-error sanity check. NOT play-tested. **Review round 4 (P0 Draw + P0/P1 reaction + P1 near-cap):** (P0 Draw starvation — real fix) round 3's `bestSpend <= max(HoldResources, EndTurn)` global stop gate was still wrong: Draw (AP-only) and holding H/E/M/T are NOT mutually exclusive, so a scarce Tech was killing Draw. HoldResources is no longer a global candidate/gate. New rule: PlayCard uses `StrategicCardEvaluator` NetScore **verbatim** (evaluator owns its own hold/scarcity); AP-only actions (Draw, AP-only Pressure) compare only against `max(EndTurn, tempoMinSpendUtility)` — the persistent-hold policy never touches them; a non-card spend (capacity upgrade) uses `eff = utility − HoldResourcesUtility(snap, itsOwnResourceCost)` — retention value of ONLY the resources it consumes. `HoldResourcesUtility` regains the `onlyConsumed` filter for exactly this. (P0/P1 reaction reservation honestly redefined) `StrategicReactionOpportunity` is now a **bounded reaction *budget***, not a fake "exact action": `{IsActionable, OwnerKey="reaction-budget:<kind>", Kind, ReservedApBudget (≤ reactionReserveApCap), Rationale, FailReason}` — no `ExactApRequired`/`ActorId`/`ResourceCost`. The estimate (cheapest responder activation + move, or cheapest affordable hand card) may legitimately exceed the ceiling → reserve the ceiling and say so in `Rationale` (never silently shrink an "exact" number). Reserves **AP only** (the replan's own affordability handles persistent resources — no phantom per-card reservation). Actionability gate keeps the "is there a responder / an affordable card at all" checks but no longer pretends that specific actor/card will execute. `ReactionOwnerStillValid(key)` → `ReactionStillActionable()`; `ExecuteRound` releases the budget if no reaction is actionable before it runs. (P1 near-cap → strategic overstock) renamed `tempoHoldOverflow*` → `tempoHoldOverstock*`; the game has **no** hard resource cap so nothing is physically lost — the term is honestly "a resource far above its runway need (`runwayTarget = max(comfortable, IncomeTarget[r] * overstockRunwayHorizon)`) is worth less to hoard", it *reduces* `holdOfConsumed` for a spend that burns that resource rather than modelling a fictitious loss. Diagnostics relabelled (`runway`/`overstock`, not `cap`/`ovf`). Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested. **Review round 5 (Reaction feasibility / reservation closure):** the reaction budget now requires a REAL feasibility probe, not "plausibility". `BuildReactionOpportunity` takes a `WorldSnapshot` and delegates to: **`ProbeTargetDriven`** — an UNCOMMITTED own field army (not in `MissionIntentRegistry` preferred-mover set, not garrison/prison/air/soloRecce/soloHeroAwaitingEscort) whose activation AP is spendable AND that can begin a safe step (`VisitHexTask.FindNextSafeStep`) toward at least one honestly-known target hex (`AiMapMemory.AllKnownEnemySightings` by id); no known hex or no reachable uncommitted responder → not feasible. **`ProbeHandFollowup`** — reuses the real pipeline preflight (`NonCombatCardPlayer.BestPlay` + `MaterializationCandidateBuilder.BestSurplus`, which already check Unit placement/capacity, Equipment host, Aviation airfield, Base/Facility slot, generation) and filters by AP ≤ available + `StrategicManager.FitsSpendableResources` (canonical ledger); generated cards use `GenerationStep.GenerationResourceCost` pre-mint. The probe returns `MinFeasibleAp` + `MinEnvelope` (persistent cost of the cheapest feasible reaction). §4: if `budget < MinFeasibleAp` → `None("budgetBelowMinimumFeasible…")` (never reserve a budget that can't cover one feasible action, never silently shrink). §5: the `Envelope` (H/E/M/T) is reserved alongside the AP budget under the same `reaction-budget:<kind>` owner, so tempo cannot spend the resources a feasible hand reaction needs. §6: `ExecuteRound` re-runs the SAME probe (`ReactionStillActionable(…, WorldAnalysis.Scan(…))`) immediately before the round and releases budget + envelope if nothing feasible remains — resources go back to the same-turn tempo re-run. `StrategicReactionOpportunity` lost `ExactApRequired`/`ActorId`/`ActionKind`-string (round-4 leftovers) and the round-3 `OwnerOf` ledger helper was removed. Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested. **Review round 6 (reaction feasibility closure):** (P0 target-driven phantom) `ProbeTargetDriven` no longer treats "an army can take a safe step" as feasible — it now requires the REAL aggression admission gate: `CombatOpportunityAnalyzer.Analyze(snap)` (stateless, no radar mutation) → `AggressionObjectiveEvaluator.Enumerate` → at least one objective with `GatePassed==true` whose `TargetArmyId` is a discovered id (so `raidTargetMaxDefenders` / `raidObjectiveMinBaseValue` / … actually gate the probe), AND an uncommitted responder that can `FindNextSafeStep` toward that objective's hex. (P1 own reservation breaks revalidation) new `StrategicResourceReservationLedger.Active(…, ignoreReason)` / `SpendableExcluding(…)`; `StrategicManager.FitsSpendableResources` gained an `excludeReason` param; `ProbeHandFollowup`'s feasibility check passes `StrategicReservationReason.StrategicReactionPass` so the §6 re-probe doesn't fail its own envelope reservation (stock==reserved → Spendable 0 → false "infeasible" → self-release). (P1 MinFeasibleAp wasn't minimum) `NonCombatCardPlayer.EnumeratePlays` + `MaterializationCandidateBuilder.EnumerateSurplusPlans` extracted (behaviour-preserving — `BestPlay`/`BestSurplus` now delegate); `ProbeHandFollowup` enumerates ALL legal plays, filters to `ap ≤ min(available, reactionReserveApCap)` + spendable envelope, and takes the genuinely cheapest — a 2-AP legal card is no longer hidden behind a 6-AP higher-scored one. (arch debt) both probes take the canonical `ActorCommitments.FromIntents(...)` (`ClaimedArmyIdSet`) built once, not a hand-rolled `PreferredMoverArmyId` scrape. `ReactionFeasibility` carries a witness (`WitnessActorId`/`WitnessTargetId`). Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested — the §12 runtime acceptance scenarios (scarce-resource Draw-wins, hand 7-8/10 → Draw, 9/10 → Play→Draw, overstock spend, reaction feasibility reserve/release-rerun, `budgetBelowMinimumFeasible`, GatePassed-rejected discovery → no reservation, envelope-protected re-probe) are still owed from a Unity run (AC20). File `Assets/Scripts/Ai/V2/StrategicTempoBudget.cs` — `.meta` carried over. **Review round 7 (P0.1 + P0.2 + P1 — reaction witness composition):** (P0.1 Discovery no longer shadows HandFollowup) `BuildReactionOpportunity` dropped the `targetDriven ? ProbeTargetDriven : ProbeHandFollowup` ternary — interrupt reasons are FLAGS, so Discovery + HandOpportunity can both be pending. It now COLLECTS `List<ReactionWitness>` from every enabled source and reserves a bounded budget for the single cheapest: rank by `MinFeasibleAp`, then `EnvelopeCost` (persistent-resource opportunity cost), then a stable `ActionKey` (Ordinal) — no fixed Discovery>Hand or Hand>Discovery priority. `ReactionFeasibility` struct → `ReactionWitness` (adds `Kind`/`ActionKey`/`NeedsResponderMove`/`OwnerKey`); each probe returns 0..N witnesses. (P0.2 GatePassed no longer an admission gate in the probe) new `ProbeMaterializationForDiscovery` — a discovered target that FAILS `AggressionObjective.GatePassed` is still a real reaction when the shortage DemandLayer would raise (Hero / FieldCombatPower) has a legal in-envelope Phase-A materialization (the case Phase A exists for: Raid accepted → GatePassed=false because power short → Demand → Phase A plays a Unit → Refresh → executable). Composes the REAL primitives — `AggressionObjectiveEvaluator.Enumerate` (accepted, no GatePassed filter) + the same covered-target / allocator-cooldown admission `DemandLayer.AggressionDemands` uses + `RaidOperationalReadiness.Evaluate` (authoritative shortage; `NeedsAssembly`→skip, real pipeline DEFERs) + `MaterializationCandidateBuilder.EnumerateSurplusPlans` gated by `CanDeliverDemandOperationally(plan, syntheticDemand)` — NOT a second planner. `ProbeTargetDriven` now takes the caller's shared stateless `CombatOpportunityReport`. Witness kind `MaterializeForDiscovery` uses the hand-style AP estimate (no responder-move add). (P1 exclude by Owner not Reason) `StrategicResourceReservationLedger.Active(…, StrategicReservationReason? ignoreReason)` / `SpendableExcluding(…, reason)` → `Active(…, string ignoreOwner)` / `SpendableExcludingOwner(…, string ignoreOwner)` (exact `Owner` match); `StrategicManager.FitsSpendableResources` param `excludeReason` → `excludeOwner`; each probe's envelope-spendable check + the §6 re-probe now exclude by the concrete `reaction-budget:<Kind>` owner, so two reaction owners sharing `Reason=StrategicReactionPass` can't shadow each other's revalidation. `ReleaseByReason` (pass-abort teardown) unchanged. Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested. **Review round 8 (P0 closure projection + P1 shared demand primitive):** (P0 — materialization witness proves CLOSURE, not the first useful step) `ProbeMaterializationForDiscovery` no longer emits a witness for "a card that improves readiness". New `ProjectMaterializationClosure`: a BOUNDED DFS (`reactionMatMaxActions`=3, pool cap 8, real branch bounds = `reactionReserveApCap` − downstream reserve, and `maxSurplus`/`maxEndOfTurnTempo`/`maxGeneration` remaining from the live `StrategicTempoBudget`) over legal `EnumerateSurplusPlans` candidates that operationally deliver the needed capability (`CanDeliverDemandOperationally` vs a synthetic Hero / FieldCombatPower demand). Accepts the subset with the smallest total prep AP that drives Σ projected `AiPower.ToPowerUnit(def).BasePower` ≥ `readiness.NumericPowerDeficit` (and/or lands ≥1 hero-delivering plan), then `MinFeasibleAp = Σ prep AP + bounded downstream AP` (`reactionResponderMoveApEstimate`, the subsequent canonical replan reserve) and `Envelope = Σ prep H/E/M/T` — only if the whole thing fits `min(reactionReserveApCap, spendable)` + spendable envelope. Does NOT try to prove projected `ReadyExecutable` (that needs a real deploy + `RefreshOperationalState`; the real DemandLayer path doesn't guarantee it either — it can go `NeedsAssembly`→DEFER). Single-step + partial power bump were phantom witnesses (Example 1: prep 2 AP + no downstream reserve → reaction AP-starved; Example 2: deficit 10, +4 card → raid still not executable). (P1 — one canonical aggression-demand primitive) new `AggressionDemandEvaluator.Build(snap, objectives, activeIntents, commitments, player)` → `AggressionDemandEvaluation {ChosenObjective, Readiness, Demands, Outcome (None/AssemblyDeferred/Demand), Reason, BlockedByCooldown, Diagnostics}`. It owns the ENTIRE admission/selection contract — covered-by-committed-raid, allocator cooldown, top-BaseValue objective pick, `RaidOperationalReadiness`, the ReadyExecutable/NeedsAssembly-DEFER/NeedsHero/NeedsPower branch, the exact `AxisDemand` shapes. Pure/deterministic: no yield, no trace ids, no logging side effect, no registry mutation (only reads `AiAllocatorStateRegistry` cooldowns); every former inline `[AI][V2][Demand][Aggression]` line is returned in `Diagnostics` for the caller to replay. `DemandLayer.AggressionDemands` is now a thin wrapper (replay diagnostics + `yield return` demands); its private `RaidKey`/`RaidDefenders` moved into the evaluator (`internal`). `StrategicReactionPass.ProbeMaterializationForDiscovery` consumes the same evaluation (`Outcome==Demand` + `ChosenObjective` in the discovered id set) — its mirrored covered/cooldown/selection helpers (`RaidKeyFor`/`RaidDefendersFor` added in round 7) deleted. New file `Assets/Scripts/Ai/V2/AggressionDemandEvaluator.cs` (+ `.meta`, + `Assembly-CSharp.csproj` Compile entry). Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested. **Review round 9 (P0.1 direct-witness + P0.2 consumption + P0.3 metric + P1 caps + minor P1):** (P0.1) the direct discovery witness no longer filters on `AggressionObjective.GatePassed` (a frozen strategic projection, not the live gate) + cheapest arbitrary pathable army (which may not be the actor that clears `RaidAssemblyPlanner`, under-reserving). `AggressionDemandEvaluator` now surfaces `ReadyExecutable: (objective, RaidAssemblyPlan)[]` — every non-covered/non-cooldown objective whose canonical `RaidOperationalReadiness.ReadyExecutable` holds (loop no longer `break`s on ready; still first-shortage-wins for the demand). `ProbeTargetDriven` builds the witness from `ReadyPlan.BaseArmyId`'s own activation AP (+ responder-move). (P0.2) new shared `MaterializationConsumptionState` (`ConsumedCards` base+equipment, `ConsumedGenerationKeys`, `ApUsed`, `Human/Energy/Materials/TechUsed`; `CardsDisjoint` / `Push`→`Token` / `Pop`). `StrategicManager.BestInjectiveAssignment` refactored onto it (behaviour-preserving — same ceilings, same check order); the reaction closure DFS uses it so one physical card that yields many `MaterializationPlan`s (Card A→Army1 / Army2 / Garrison) can no longer be counted as two bumps, and a generation `CardKey` / last-Tech can't be double-spent across the subset. (P0.3) the closure no longer sums raw `AiPower.ToPowerUnit(def).BasePower` (wrong unit vs the shortage). It projects the delta of the SAME metric `NumericPowerDeficit` is in: per target army (Existing/Shell/New) it keeps a seed roster (`ArmySnapshot.MembersWithHeroes`) + a raid-eligibility flag mirroring `CapabilityInventory` (`IsStructuralRaidActor` && unclaimed), applies every chosen plan's projected profile (`AiPower.ProjectMaterialization` for equipped chains, else `ToDefenderProfile`) SEQUENTIALLY, and `contribution = Σ over eligible armies (EffectiveArmyPowerFromProfiles(seed+added) − seed)` — composition multiplier, multiple materializations into one army, NewArmy vs ExistingArmy, and "one unit doesn't make the army raid-eligible" all fall out correctly. (P1) reaction preparation IS Phase A → the DFS action bound is `maxDemandFulfillmentActionsPerTurn`, NOT `maxSurplusActionsPerTurn` / `maxEndOfTurnTempoActionsPerTurn` (the end-of-turn tempo caps `FulfillDemands` never debits); generation stays bounded by the turn-scoped generation budget (Phase A does debit that). (minor P1) `AggressionDemandEvaluator.Build` uses new `AiAllocatorStateRegistry.Peek` (non-creating) instead of `GetOrCreate` — genuinely no registry mutation; null == no cooldowns (behaviour-equivalent to the old fresh-empty state). New file `Assets/Scripts/Ai/V2/MaterializationConsumptionState.cs` (+ `.meta` + csproj Compile entry). Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested. **Review round 10 (P0.1 full RequiredAp + P0.2 canonical own-force metric + P0.3 recipient capacity + P1 pool/depth):** (P0.1) `ReactionWitness` dropped `MinFeasibleAp` + `NeedsResponderMove`; it now carries ONE `RequiredAp` = the full cost of the protected reaction (direct: activation + `reactionResponderMoveApEstimate`; materialization: prep + downstream; hand: `max(play, reactionFollowupApEstimate)`). `BuildReactionOpportunity` ranks and gates on that exact number and reserves it verbatim — a witness whose `RequiredAp` exceeds the ceiling is DROPPED, never clamped to a smaller "budget" that fails to protect the action, and ranking is no longer done pre-move-cost. (P0.2) the closure stopped projecting own-force power through `WorthIt.DefenderProfile` / `EffectiveArmyPowerFromProfiles` (a lossy enemy/fog line — no Resistance/Fate, Range→1, IsHero=false, reduced abilities), which is a different metric than the `ArmySnapshot.EffectiveArmyPower` `NumericPowerDeficit` is built from. It now projects through `AiPower.PowerUnit` + `AiPower.EffectiveArmyPower`: seed roster = live `ArmyData.Members` → `AiPower.ToPowerUnit`; each plan's unit = `AiPower.ProjectMaterialization(plan)` → `PowerUnit(BasePower, tags, Range, IsHero)` (one path for every chain kind, equipment folded in). Same metric on both sides of the ≥ comparison. (P0.3) new per-recipient-army capacity projection in the DFS (`RecipientCapacityOk`, checked before Push): canonical `ArmyData.Capacity − Members.Count` free-slot baseline + hero occupancy, so two individually-legal cards into the SAME 1-slot army can't BOTH be accepted (execution would preflight-fail the second); one hero per army; a hero raises capacity by a conservative +1; the combined generate-chain `HandSlotsNeededAtPeak` must fit `hand.Capacity − hand.Hand.Count`. Conservative → false-negative, never phantom. (P1) `reactionMatPoolCap` 8→24 (a pure DoS width valve — at realistic hand sizes it never truncates; if it did, false-negative only), and the DFS DEPTH bound is now the single `AiConfigV2.maxDemandFulfillmentActionsPerTurn` (the exact per-call limit the real reaction `FulfillDemands` runs under) — the `reactionMatMaxActions` local const is gone. Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested. |

| AI-MGR-03 | Context-Aware Aviation Sortie Reservation (Recon) | DONE (Recon only; Combat sortie deferred) — new `AviationSortieReservationEvaluator.cs` (Assets/Scripts/Ai/V2/Recon/): pure/read-only `EvaluateRecon(...)` runs the 4-stage pipeline (Resource Outlook → Hand/Deck Energy Pressure → Sortie Value → Reservation Decision) and returns an `AviationReservationDecision` struct (`ShouldReserve`, `SortieType`, `ReserveAp/Energy`, `ResourceHeadroom`, `HandEnergyPressure`/`DeckEnergyPressure`/`ProtectedCardEnergy`, `ReconUtility`/`CombatUtility`(=0, deferred)/`SelectedSortieUtility`, `OpportunityCost`, `Reason`) plus a one-line `[AviationReserveEval]` diagnostic. Stage 1 (Resource Outlook) reads `root.GetResource(Energy)`/`root.ActionPoints` directly and nets out other in-flight wings' owed Energy + `extraCommittedEnergy` (same-pass prior candidates), and folds `IncomeProjection.IncomeFor` into a bounded-horizon `EnergyHeadroom`. Stage 2 reuses `ReconAirEnergyPolicy.ProtectedHandEnergy`/`ProtectedNearTermDrawEnergy` (now `internal`, unchanged generic name-free logic) as `HandEnergyPressure`/`DeckEnergyPressure` — no new card-Energy model, no hardcoded card names. Stage 3 takes the AIR-01 route score (`AirReconRouteScorer` via `ReconAirStepPlanner.Pick`/`PickFromStorage`) VERBATIM as `ReconUtility` — that score already composes freshness + enemy/frontline/objective proximity + frontier coverage + actionable value, so `InformationAge != InformationValue` is inherited, not re-derived. Stage 4 gates AP headroom, then Energy headroom net of `ProtectedCardEnergy`, then a soft opportunity-cost term (mirrors the retired `ReconAirEnergyPolicy` soft term) before `ShouldReserve=true`; distinct fail reasons (`insufficient_ap_headroom` / `insufficient_energy_headroom` / `energy_needed_by_hand` / `future_energy_pressure` / `no_actionable_recon_value` / `sortie_below_opportunity_cost`) and accept reasons (`valuable_recon_opportunity` / `resource_surplus` / `sortie_overrides_card_pressure`). Wired into `ReconAirReservationPrepass.SlotWouldFly` (`ReconAirReservation.cs`) — replaces the old `choice.Value.Score < MinimumUsefulScore` + `ReconAirEnergyPolicy.Evaluate(...).Allowed` pair with one evaluator call + its own `AiDebugLog` line; a route below `aviationReserveMinSortieUtility` (= `airReconMinimumUsefulScore`) is still rejected first, so no behavioural regression there. Deliberately NOT touched (execution-time gates, out of "reservation" scope, real dependents kept working unchanged): `AirReconStepDirector`'s per-step activation-Energy check, `AirReconPlanner`'s plan-time launch gate, `ReconAirCapacityPolicy.EvaluateDetailed`'s budget-base probe — all three still call `ReconAirEnergyPolicy.Evaluate` directly (unchanged signature/behaviour). New `AiConfigV2` block `aviationReserve*` (`aviationReserveIncomeHorizon=3`, `aviationReserveMinSortieUtility=airReconMinimumUsefulScore`, `aviationReserveOpportunityWeight=0.5`, `aviationReserveMinNetUtility=0`). New file `Assets/Scripts/Ai/V2/Recon/AviationSortieReservationEvaluator.cs` (+ `.meta` + `Assembly-CSharp.csproj` Compile entry). Combat sortie value (spec §3 Combat) intentionally deferred — user confirmed scope is "full evaluator for aviation, recon first, combat sorties added later." Both assemblies build clean (0/0) via `dotnet build -t:Rebuild`. NOT play-tested. |

Suggested order (dependency-driven): INTEL-01 → RECON-02 → RECON-01 → AIR-01 → AIR-02,
then MGR-01 → MGR-02 → MGR-03 (MGR track is file-independent from recon/air except `StrategicManager`/`ReconAirReservation`).
